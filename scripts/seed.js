...
async function main() {
  ...
  try {
    ...
    const { monfairContract: monfairAddress } = JSON.parse(contractAddresses)

    monfairContract = await ethers.getContractAt('MonFair', monfairAddress)

    // Process #1
    await Promise.all(
      generateGameData(dataCount).map(async (game) => {
        await createGame(monfairContract, game)
      })
    )
        ...
        .map(async (_, i) => {
          ...
          await Promise.all(
            invitations.map(async (player) => {
              await sendInvitation(monfairContract, player)
            })
          )
        })
    )
    ...
    // Process #3
    await getGames(monfairContract)
    await getInvitations(monfairContract, 1)
    await getMyInvitations(monfairContract)
  } catch (error) {
